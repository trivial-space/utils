...
export function makePerspective(
	...
	const perspectiveSettings = val<PerspectiveSettings>({
		...
		fovy: Math.PI * 0.6,
		aspect: 1
	}).react([canvasSize.HOT], (self, { width, height }) => {
		self.aspect = width / height
		return self
	})

	const perspective = val(mat4.create()).react(
...
export function makeFirstPersonView() {
	...
	const rotation = val({
		...
		.react([rotY.HOT], (self, rotY) => {
			mat4.fromYRotation(self.rotY, rotY)
			return self
		})
		.react([rotX.HOT], (self, rotX) => {
			mat4.fromXRotation(self.rotX, rotX)
			return self
		})

	const view = val(mat4.create()).react(

...
import {
	...
	FormData,
	DrawSettings
} from 'tvs-painter/dist/lib/painter-types'
...
export function createBodyCanvas() {
	const canvas = asyncStreamStart<HTMLCanvasElement>(null, send => {
...
export function setupPainter(
	...
	const painter = asyncStream<Painter, GL>([gl.HOT], (send, gl) => {
	...
	painter.react([canvasSize.HOT], (p, _) => p.resize(), 'updateSize')
...
export function makeShadeEntity(
	...
	const entity = asyncStream<Shade, Painter>([painter.HOT], (send, painter) => {
...
export function makeFormEntity(
	...
	const entity = asyncStream<Form, Painter>([painter.HOT], (send, painter) => {
...
export function makeStaticLayerEntity(
	...
	const entity = asyncStream<Layer, Painter>([painter.HOT], (send, painter) => {
...
export function makeEffectLayerEntity(
	...
	const entity = asyncStream<Layer, Painter>([painter.HOT], (send, painter) => {
		const layer = painter
			.createEffectLayer()
			.update({ frag: defaultShaders.basicEffect.frag })
		send(layer)
		return layer.destroy
	})
